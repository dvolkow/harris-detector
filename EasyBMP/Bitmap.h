...
public:
	...
	CBitmap(unsigned int dimX, unsigned int dimY) : m_BitmapData(0), m_BitmapSize(0)  {
		Dispose();

		m_BitmapSize = dimX * dimY;
	...
	RGBA GetAt(unsigned int x, unsigned int y)
	{
		x = ((x >= GetWidth()) ? GetWidth() - 1 : x);
		y = ((y >= GetHeight()) ? GetHeight() - 1 : y);

		return m_BitmapData[GetWidth() * (GetHeight() - y - 1) + x];
	}

	void SetTo(unsigned int x, unsigned int y, RGBA value)
	{
		if (x >= GetWidth()) return;
		if (y >= GetHeight()) return;

		m_BitmapData[GetWidth() * (GetHeight() - y - 1) + x] = value;
	}

	BITMAP_HEADER GetHeader()
	{
		return m_BitmapHeader;
	}

	void SetHeader(BITMAP_HEADER value)
	{
		m_BitmapHeader = value;
	}
	...
	void SetFileHeader(BITMAP_FILEHEADER value)
	{
		m_BitmapFileHeader = value;
	}
};
